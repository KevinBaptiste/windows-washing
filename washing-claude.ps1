...
# Les variables marquées $Script: sont passées en arguments pour rester portables.
...
function Invoke-DebloatStep {
    ...
    $runOk = Invoke-WithRetry -Label "$label (exec)" -Action {
        ...
        Write-LogEntry -Message "Win11Debloat stdout (extrait) : $($output.Substring(0, [Math]::Min(500, $output.Length)))" -Level INFO
    }
...
function Install-Microsoft365 {
    ...
    $odtOk = Invoke-WithRetry -Label "$label (ODT install)" -Action {
        ...
        if ($LASTEXITCODE -ne 0) { throw "Winget ODT ExitCode = $LASTEXITCODE" }
    }
...
function Get-AntivirusStatus {
    ...
    try {
        ...
        foreach ($av in $products) {
            # Décodage du bitmask productState (cf. Microsoft Learn).
...
function Install-GoogleChrome {
    ...
    $ok = Invoke-WithRetry -Label $label -Action {
        $args = @('install', '--id', 'Google.Chrome',
                  '--silent', '--accept-source-agreements', '--accept-package-agreements')
        & winget @args | Out-Null
        # Winget renvoie 0 si installé, -1978335189 si déjà présent (no upgrade) : on tolère.
...
# --- Auto-suppression différée du script PS7 lui-même (détaché) ---
$selfDelete = "timeout /t 5 /nobreak >nul & del /f /q `"$SelfPath`""
Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', $selfDelete -WindowStyle Hidden
 
#endregion ===========================================================================
'@
 
# Écriture du script PS7 en UTF-8 avec BOM (accents PS7 OK, prudence accrue).
